"""
This script translates a column of text in a DataFrame using a vLLM-hosted Gemma model.
"""
...
def unload_model(model):
    ...
    if model is not None:
        if hasattr(model, 'llm_engine') and hasattr(model.llm_engine, 'model_executor') and hasattr(model.llm_engine.model_executor, 'driver_worker'):
            try:
...
def build_prompt_gemma_instruct(src_lang_name: str, tgt_lang_name: str, text: str) -> str:
    """Builds a prompt for the Gemma Instruct model."""
    user_query = (
        f"Translate the following text from {src_lang_name} into {tgt_lang_name}. "
        f"Provide ONLY the translated text. Do not include any introductory phrases, explanations, "
        f"or the original language tags in your response.\n\n"
        f"{src_lang_name}: {text}\n\n"
        f"{tgt_lang_name}:"
    )
    return (
        f"<start_of_turn>user\n"
        f"{user_query.strip()}<end_of_turn>\n"
...
def stream_batch_translate_gemma_instruct(df_path: str,
                                 rows: int,
                                 output_path: str,
                                 column: str,
                                 src_lang_name: str,
                                 ...
                                 batch_size: int = 4,
                                 export: bool = True):
    """Translates a DataFrame column using a Gemma Instruct model."""
    print(f"--- {model_name_arg} Model Translation Job ---")
    ...
    try:
        ...
        for i in tqdm(range(0, len(df), batch_size), desc=f"Translating with {model_name_arg}"):
            ...
            for target_language_name in target_lang_names:
                ...
                messages_batch = [build_prompt_gemma_instruct(src_lang_name, target_language_name, text)
        ...
        if llm:
            ...
            print(f"\nProcessing finished for {model_name_arg}. Model was not loaded or already handled.")
    ...
    if export:
        try:
            ...
            safe_model_name = model_name_arg.replace("/", "_").replace("-", "_").replace(".", "_")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Translate a DataFrame column using a vLLM-hosted Gemma model.")
    ...
    parser.add_argument("--model_name", type=str, default="Gemma-3-12B-IT", help="Name of the model.")
    parser.add_argument("--model_repo", type=str, default="google/gemma-3-12b-it", help="Repo of the model.")
    ...
    if not os.path.exists(args.df_path):
        ...
        data = {args.column: [
            ...
            "This is a test sentence for the new Gemma model.",
            "Machine translation is an interesting field of artificial intelligence.",
            "The weather is beautiful, with clear skies and a gentle breeze.",
            "Let's try a significantly longer sentence to thoroughly evaluate the model's capabilities in handling more complex input structures, including various pieces of specific terminology such as 'computational linguistics' and 'deep learning architectures'.",
            "Another short example to process.",
            "One final test before concluding this batch."
        ]}
        dummy_df = pd.DataFrame(data)
        dummy_df.to_pickle(args.df_path)
        print(f"Dummy DataFrame saved to {args.df_path}")

    translations_df = stream_batch_translate_gemma_instruct(
    ...
    if not translations_df.empty:
        print("\n--- Translation Summary ---")
        print(f"Generated {len(translations_df)} translation records.")
        ...
        print("\nTranslations with errors (if any):")
        error_df = translations_df[translations_df['trs_prompt'].str.startswith("ERROR:", na=False)]
        if not error_df.empty:
            print(error_df)
        else:
            print("No errors recorded in translations.")
        print("\nSkipped/Empty original prompts (if any):")
        ...
        if not skipped_df.empty:
            ...
            print("No skipped or originally empty prompts found.")
    else:
