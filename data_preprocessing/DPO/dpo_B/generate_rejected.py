...
def build_prompt(prompt_text: str) -> str:
    """Builds a prompt string in Qwen2 Instruct format."""
    system_message = "You are Qwen, created by Alibaba Cloud. You are a helpful assistant."
    return (
        f"<|im_start|>system\n"
        f"{system_message}<|im_end|>\n"
        f"<|im_start|>user\n"
        f"{prompt_text.strip()}<|im_end|>\n"
        f"<|im_start|>model\n"
    )
...
def generate_batch(llm, prompts_batch: List[str], max_tokens: int, temperature: float) -> list:
    ...
    sampling_params = SamplingParams(max_tokens=max_tokens, temperature=temperature, stop=["<|im_end|>", "<|endoftext|>"])
...
def main():
    ...
    parser.add_argument("--output_filename", type=str, default="prompt_responses_rejected.json", help="Name of the output file.")
    ...
    all_generations = []
    for i in tqdm(range(0, len(df), args.batch_size), desc="Generating responses"):
